...
**Текущая ситуация:**
В настоящий момент компания сталкивается с рядом ограничений из‑за отсутствия полноценного мониторинга:

* проблемы выявляются только после жалоб клиентов (реактивный подход);

* для определения причин потери заказов требуется ручной анализ;

* отсутствуют данные о производительности системы;

* мониторинг Яндекс Метрики охватывает только B2C веб‑интерфейс — B2B API остаётся без контроля.

**Бизнес-риски отсутствии мониторинга:**
1. Потеря клиентов и контрактов из‑за незамеченных вовремя сбоев.

2. Долгое время реагирования на инциденты (часы или дни вместо минут).

3. Невозможность грамотно планировать масштабирование системы.
...
* **обнаруживать проблемы проактивно** — получать алерт до того, как их заметят клиенты;

* **сократить MTTR (Mean Time To Recovery)** с часов до минут;

* **планировать мощности (capacity planning)** на основе объективных данных;

* **соблюдать SLA** и гарантировать уровень сервиса партнёрам;

* **обосновывать инвестиции** в развитие инфраструктуры с опорой на фактические показатели.

### ROI мониторинга

* **Текущие потери:** потеря нескольких крупных контрактов, рост недовольства клиентов.

* **Затраты на внедрение:** расходы на инфраструктуру и время команды.

* **Ожидаемый эффект:** предотвращение будущих потерь, сокращение времени простоя системы.
...
Для обеспечения эффективного мониторинга различных компонентов системы используются специализированные методологии, подобранные с учётом специфики каждого элемента:

| Компонент | Подход | Обоснование |
|-----------|--------|-------------|
| **API-сервисы** (Shop, CRM, MES) | **RED** (Rate, Errors, Duration) | Оптимален для request‑driven сервисов: позволяет сфокусироваться на качестве пользовательского опыта и оперативно выявлять проблемы с запросами |
| **Базы данных** (PostgreSQL) | **USE** (Utilization, Saturation, Errors) | Подходит для ресурсов с ограничением по capacity: даёт полную картину загрузки и насыщения системы |
| **RabbitMQ** | **USE + специфичные** | Поскольку очередь — это ресурс, USE обеспечивает базовый контроль, а дополнительные метрики очередей (например, глубина очереди, скорость обработки) дают более детальную картину |
| **S3 хранилище** | **USE** | Подходит как для ресурса с ограничением по объёму: позволяет отслеживать использование пространства и выявлять потенциальные проблемы с доступностью |
| **Инфраструктура** (EC2) | **USE** | Классический подход для мониторинга физических и виртуальных серверов: охватывает ключевые показатели загрузки и производительности |

### Почему не только "Четыре золотых сигнала"?

«Четыре золотых сигнала» (Latency, Traffic, Errors, Saturation) — это упрощённая методология, подходящая для базового мониторинга. В нашей системе мы применяем более детальные подходы (RED и USE), поскольку они:

предоставляют расширенный контекст для диагностики проблем;

* позволяют точнее локализовать источник сбоев;

* учитывают специфику разных компонентов системы;

* дают больше данных для превентивного выявления потенциальных проблем.
...
При выборе инструментов для системы мониторинга учитывались надёжность, функциональность и экономическая эффективность. 

**Итоговый набор решений:**

| Компонент | Технология | Альтернативы | Обоснование выбора |
|-----------|------------|--------------|-------------------|
| Metrics DB | **Prometheus** | VictoriaMetrics, InfluxDB | Prometheus — индустриальный стандарт с богатой экосистемой инструментов; решение бесплатное, что снижает затраты на внедрение |
| Visualization | **Grafana** | Kibana, DataDog | Бесплатное решение с нативной интеграцией с Prometheus, гибкими возможностями настройки дашбордов |
| Alerting | **Alertmanager** | PagerDuty native, Opsgenie | Является частью Prometheus stack, обеспечивает гибкую маршрутизацию алертов и простую настройку правил оповещения |
| Long-term storage | **Thanos** / **VictoriaMetrics** | Cortex | Позволяют хранить метрики дольше 15 дней, обеспечивая масштабируемость и надёжность хранения исторических данных |