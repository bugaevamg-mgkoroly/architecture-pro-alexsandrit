...
Команда «Александрит» сталкивается с рядом критических проблем в обработке заказов:

* заказы оказываются в неопределённом состоянии или зависают на каком‑либо этапе без видимых причин;

* сообщения теряются при передаче между сервисами (например, от Shop API к RabbitMQ, далее к CRM/MES);

* невозможно точно установить, на каком этапе обработки «застрял» конкретный заказ;

* разбор инцидентов занимает дни вместо минут, что снижает оперативность реагирования.

### Почему нужен трейсинг

**Distributed tracing (распределённый трейсинг)** позволяет отслеживать полный путь запроса или заказа через все компоненты распределённой системы. В отличие от логов (фиксируют точечные события) и метрик (предоставляют агрегированные данные), трейсинг даёт:

* полную картину маршрута запроса через все сервисы;

* данные о времени выполнения каждого этапа обработки;

* точную точку возникновения ошибки или задержки;

* понимание причинно‑следственных связей между вызовами сервисов.
