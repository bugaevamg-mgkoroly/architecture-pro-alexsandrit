...
**Почему observability — приоритет № 1**
* Отсутствие мониторинга, трейсинга и логирования делает невозможным выявление точек потери или зависания заказов.
* Внедрение observability позволит точно определить реальные узкие места (bottlenecks) в системе.

**Приоритет кеширования над масштабированием обоснован следующими факторами**
* Проблема чётко локализована (торможение дашборда в MES), что позволяет точечно применить решение.
* Внедрение кеширования даст быстрый результат: эффект будет заметен почти сразу, а доработки системы минимальны.
* В отличие от этого, масштабирование — более трудоёмкий процесс: оно требует серьёзных изменений инфраструктуры и занимает больше времени.

**Почему DLQ критичен?**
* Любая потеря сообщений напрямую оборачивается потерей заказов. Это ведёт не только к финансовым убыткам, но и к снижению доверия клиентов и ущербу репутации компании.
* При этом настройка DLQ не требует масштабных доработок: относительно небольшие усилия на внедрение обеспечивают надёжную защиту от потерь и существенно повышают стабильность системы.
